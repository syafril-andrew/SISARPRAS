...
class UserController extends SecureController
{
	...
	function add($formdata = null)
	{
		if ($formdata) {
			...
			if ($db->has($tablename)) {
				$this->view->page_error[] = $modeldata['username'] . " Already exists!";
			}
			// Check if Duplicate Record Already Exists In The Database
			...
			if ($this->validated()) {
				// Set user_role_id value to 1
				$modeldata['user_role_id'] = 1;
	...
	function edit($rec_id = null, $formdata = null)
	{
		...
		$tablename = $this->tablename;
		//editable fields
		$fields = $this->fields = array("id_user", "username", "nama", "photo", "user_role_id");
		if ($formdata) {
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'username' => 'required',
